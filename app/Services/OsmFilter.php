...
class OsmFilter
{
    ...
    public static function test_bicycleundefined($tags)
    {
        ...
        if ($designated && $no_surface) {
            return true;
        }// для велосипедов, но нету вообще информации о качестве покрытия
    ...
    public static function test_bikelane($tags)
    {
        ...
        if (! empty($tags['cycleway']) && in_array($tags['cycleway'], $lanes)) {
            return true;
        }
        if (! empty($tags['cycleway:left']) && in_array($tags['cycleway:left'], $lanes)) {
            return true;
        }
        if (! empty($tags['cycleway:right']) && in_array($tags['cycleway:right'], $lanes)) {
            return true;
        }
        if (! empty($tags['cycleway:both']) && in_array($tags['cycleway:both'], $lanes)) {
            return true;
        }
    ...
    public static function test_greatother($tags)
    {
        ...
        if (! empty($tags['highway']) && $tags['highway'] == 'track' && $surface) {
            return true;
        }
    ...
    public static function test_no($tags)
    {
        ...
        $no_surface = self::no_surface_information($tags);
        if ($no_great_tags && $tags['highway'] != 'track') {
            return true;
        }// никакого упоминания ни велосипедов, ни людей.
        if ($no_surface && $no_great_tags && $tags['highway'] == 'track') {
            return true;
        }// нету покрытия и это трек без велосипедов и ходьбы
        $filters = [
    ...
    public static function test_element($element)
    {
        ...
        if (! empty($element['tags']['lbroads'])) {
            return $element['tags']['lbroads'];
        }
        if (self::test_no($element['tags'])) {
            return 'no';
        }
        if (self::test_great($element['tags'])) {
            return 'great';
        }
        if (self::test_bicycleundefined($element['tags'])) {
            return 'bicycle_undefined';
        }
        if (self::test_bikelane($element['tags'])) {
            return 'bikelane';
        }
        if (self::test_greatother($element['tags'])) {
            return 'greatfoot';
        }
        if (self::test_foot($element['tags'])) {
            return 'foot';
        }
